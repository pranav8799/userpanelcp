...
import StepCalculator from "@/components/reports/StepCalculator";
...
export default function Reports() {
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      data.push({ date: format(date, "MMM dd"), fullDate: date, pnl: dayPnl, isProfit: dayPnl >= 0 });
    }
    return data;
  }, [orders, days]);
  ...
  return (
    <div className="space-y-6 h-full flex flex-col pb-8">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Performance and calculators</p>
      </div>
      ...
      <Tabs defaultValue="performance" className="flex-1 flex flex-col min-h-0">
        ...
        <TabsContent value="performance" className="flex-1 flex flex-col space-y-6 mt-4">
                ...
                <div className="h-[350px] w-full px-2">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} minTickGap={20} />
                      <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} tickFormatter={(val) => `$${Math.abs(val)}`} />
                      <Tooltip
        ...
        <TabsContent value="calculator" className="flex-1 mt-4 overflow-y-auto">
          <StepCalculator />
        </TabsContent>
      </Tabs>
    </div>
