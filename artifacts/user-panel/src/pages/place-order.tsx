...
type ConfirmState =
  ...
  | { type: "ladder_reset_toggle"; enabling: boolean }
  | null;
...
interface RepunchFilters { search: string; side: "ALL" | "BUY" | "SELL"; status: "ALL" | "queued" | "pending_fill" | "placing_tp" | "watching" | "repunching" | "stopped"; }
...
// Mirrors roundPrice in repunchEngine.ts — the backend compares prices with
// strict equality to detect duplicate/colliding legs, so the frontend must
// hand it the same "clean" rounding, or two legs that look identical on
// screen (small step sizes like 0.01 cause float drift) get tracked as
// different prices.
const roundPrice = (v: number) => Math.round(v * 1e8) / 1e8;
...
// A queued leg is a pending_fill slot that hasn't been given an orderId yet.
const isQueued = (slot: WatchedSlot) => slot.status === "pending_fill" && !slot.orderId && !slot.stopped;

const slotStatusLabel = (slot: WatchedSlot): string => {
  if (slot.stopped) return "Stopped";
  if (isQueued(slot)) return "Queued";
  switch (slot.status) {
...
function ConfirmDialog({ state, onConfirm, onCancel }: { state: ConfirmState; onConfirm: () => void; onCancel: () => void }) {
  ...
  const cfg = {
    ...
    ladder_reset_toggle: {
      title: state.type === "ladder_reset_toggle" && state.enabling ? "Enable Ladder Reset" : "Disable Ladder Reset",
      desc: state.type === "ladder_reset_toggle"
        ? (state.enabling
            ? "If price runs away with nothing filled, or the first order's TP fills, the ladder will be torn down and rebuilt closer to market. Continue?"
            : "Legs will stay exactly where they were placed — no chase reset, and a first-order TP fill will only re-punch that single leg instead of rebuilding the whole ladder. Continue?")
        : "",
      label: state.type === "ladder_reset_toggle" && state.enabling ? "Yes, Enable" : "Yes, Disable",
    },
    ...
    <div className="md:hidden fixed inset-0 z-[60] flex flex-col" style={{ background: "hsl(var(--background))" }}>
      ...
      <div className="px-4 py-2.5 space-y-2 shrink-0" style={{ borderBottom: "1px solid hsl(var(--border))" }}>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <FilterChip label="Status" value={filters.status} options={[{ value: "ALL", label: "All Statuses" }, { value: "queued", label: "Queued" }, { value: "pending_fill", label: "Pending Fill" }, { value: "placing_tp", label: "Placing TP" }, { value: "watching", label: "Watching" }, { value: "repunching", label: "Re-punching" }, { value: "stopped", label: "Stopped" }]} onChange={(v) => setFilters((f) => ({ ...f, status: v as RepunchFilters["status"] }))} activeColor="hsl(162 88% 42%)" />
          {activeFilterCount > 0 && (
            <button onClick={onClearFilters} className="flex items-center gap-1 px-2 py-1 rounded-md text-[11px] font-semibold transition-all"
              style={{ color: "hsl(345 88% 62%)", border: "1px solid hsl(345 88% 58% / 0.3)", background: "hsl(345 88% 58% / 0.07)" }}>
...
export default function PlaceOrder() {
  ...
  const [stepSizeIncrement, setStepSizeIncrement] = useState("2"); // 8c. chase-reset multiplier (1-5), default 2
    const [doubleQtyEnabled, setDoubleQtyEnabled] = useState(true); // 8b. double qty for far half — per-trade, default on
  const [ladderResetEnabled, setLadderResetEnabled] = useState(true); // 8d. ladder reset (chase + rank-0 TP rebuild) — per-trade, default on
  ...
  useEffect(() => {
    if (serverConfig && !defaultsLoaded) {
      ...
      if (serverConfig.ladderResetEnabled !== undefined) setLadderResetEnabled(serverConfig.ladderResetEnabled);
 ...
 /* ── multi-order limit ladder, placed after the entry order fills ──
     Only CONCURRENT_LIMIT ladder legs rest on the book at once. The rest are
     stored as "queued" and get activated one at a time by the backend
     engine (repunchEngine.ts) as each active leg's entry fills. ── */
  ...
  const runAutoPunch = useCallback(async (
    ...
    doubleQtyEnabled = false,
    ladderResetEnabled = true,
  ) => {
    setIsPunching(true);

    const batchId = `${tradeSymbol}-${tradeSide}-${Date.now()}`;
    let totalOk = 0, totalFailed = 0;
    const newSlots: WatchedSlot[] = [];

    // Rank 0 = entry, rank 1..orderCount = ladder legs in order. First half
    // (rounded up) trades at baseQty, the rest at baseQty*2 — only while
    // doubleQtyEnabled is on. Matches the engine's computeQtyForRank in
    // repunchEngine.ts, which re-derives qty the same way on every future
    // repunch/shift/activation for this batch.
    const totalLegs = 1 + cfg.orderCount;
    ...
    const stepSizeIncrementClamped = Math.max(1, Math.min(100, cfg.stepSizeIncrement || 2));

    // Entry leg — placed immediately, not part of the ladder concurrency window.
    if (entryLeg) {
      const tp0 = roundPrice(tradeSide === "BUY" ? tradeEntryPrice + cfg.tpPoints : tradeEntryPrice - cfg.tpPoints);
      newSlots.push({
    ...
    // Ladder legs: place only the first CONCURRENT_LIMIT immediately.
    // The rest go in as "queued" and get released 1-for-1 as active legs fill.
    for (let n = 1; n <= cfg.orderCount; n++) {
      const limitPrice = roundPrice(tradeSide === "BUY"
        ? tradeEntryPrice - cfg.stepSize * n
        : tradeEntryPrice + cfg.stepSize * n);
      const tp = roundPrice(tradeSide === "BUY"
        ? limitPrice + cfg.tpPoints
        : limitPrice - cfg.tpPoints);
      const qty = qtyForRank(n);
      ...
      if (n <= CONCURRENT_LIMIT) {
        try {
          ...
          newSlots.push({
            ...
            baseQty,
            totalLegs,
            rank: n, // non-first → individual re-punch only on TP
          });
          totalOk++;
        } catch (err) {
          totalFailed++;
        ...
        // Queued leg: valid "pending_fill" status but no orderId yet — the
        // engine activates it (assigns orderId) once an earlier leg in this
        // batch fills. See activateNextQueued in repunchEngine.ts.
        newSlots.push({
          id: `${tradeSymbol}-${tradeSide}-${limitPrice}-${Date.now()}-${n}`,
          symbol: tradeSymbol,
          side: tradeSide,
    ...
    const queuedCount = Math.max(0, cfg.orderCount - CONCURRENT_LIMIT);
    toast({
      title: totalFailed === 0
        ? `⚡ ${totalOk} order${totalOk !== 1 ? "s" : ""} placed${queuedCount > 0 ? `, ${queuedCount} queued` : ""}`
    ...
    try {
      ...
      if (result && orderCount >= 1 && stepSizeNum > 0) {
        ...
        if (!ep || isNaN(ep)) {
          ...
          const stepSizeIncrementNum = Math.max(1, Math.min(100, parseInt(stepSizeIncrement) || 2));
          await runAutoPunch(symbol.toUpperCase(), side, ep, baseQty, { orderCount, stepSize: stepSizeNum, tpPoints: tpPointsNum, stepSizeIncrement: stepSizeIncrementNum, ladderResetEnabled }, { orderId: result.orderId }, doubleQtyEnabled, ladderResetEnabled);
        }
      }
      void refetchOrders();
      void refetchPositions();
  ...
  }, [symbol, quantity, price, side, orderType, numberOfOrders, stepSize, takeProfit, stepSizeIncrement, doubleQtyEnabled, ladderResetEnabled, runAutoPunch, toast, refetchOrders, refetchPositions]);
  const handleSaveDefaults = useCallback(() => {
  const cfg: AutoPunchConfig = {
    ...
    stepSizeIncrement: Math.max(1, Math.min(100, parseInt(stepSizeIncrement) || 2)),
    ladderResetEnabled,
  };
  setIsSavingDefaults(true);
  updateSettingsMut.mutate({ data: { autoPunchConfig: cfg } }, {
...
}, [numberOfOrders, stepSize, takeProfit, stepSizeIncrement, ladderResetEnabled, updateSettingsMut, toast, handleExecute]);
  ...
  const handleConfirm = useCallback(() => {
    ...
    else if (confirmState.type === "ladder_reset_toggle") setLadderResetEnabled(confirmState.enabling);
  ...
  const filteredSlots = useMemo(() => {
    ...
    return watchedSlots.filter((slot) => {
      ...
      if (repunchFilters.status !== "ALL") {
        ...
        else if (repunchFilters.status === "queued") { if (!isQueued(slot)) return false; }
        else { if (slot.stopped || isQueued(slot) || slot.status !== repunchFilters.status) return false; }
      }
      return true;
    });
...
const topBottomOutLevels = Array.from({ length: 10 }, (_, i) => i + 1); // 1..10
// Same rank->qty logic as runAutoPunch's qtyForRank, so the margin preview
// matches what will actually get placed once Double Qty is on.
const baseQtyNum = parseFloat(quantity) || 0;
const baseCount = Math.ceil(totalLegs / 2);
const qtyForRankPreview = (rank: number) =>
  doubleQtyEnabled && rank >= baseCount ? baseQtyNum * 2 : baseQtyNum;

const marginPerUnit = price && leverage ? parseFloat(price) / leverage : null; // margin for 1 unit of qty
const requiredMargin = marginPerUnit != null && !isNaN(marginPerUnit) && baseQtyNum > 0
  ? Array.from({ length: totalLegs }, (_, rank) => qtyForRankPreview(rank))
      .reduce((sum, qty) => sum + qty * (marginPerUnit as number), 0)
  : null;
...
{stepSizeNum > 0 && numberOfOrdersNum > 0 && (<p className="text-[10px] -mt-2" style={{ color: "hsl(258 82% 60%)" }}>⚡ Will ladder {numberOfOrdersNum} limit{numberOfOrdersNum !== 1 ? "s" : ""} from this price after entry.</p>)}
            ...
            {/* 6/7/8. Buy Diff + Take Profit + Orders — single row */}
            <div className="grid grid-cols-3 gap-2">
              <div>
                <label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground mb-1 block">Buy Diff</label>
                <input
            ...
            {/* Ladder Reset + Top/Bottom Out + Double Qty — single row */}
            <div className="grid grid-cols-3 gap-2">
              <div>
                <label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground mb-1 block">Auto Shift</label>
                <button
                  type="button"
                  onClick={() => setConfirmState({ type: "ladder_reset_toggle", enabling: !ladderResetEnabled })}
                  className="w-full flex items-center justify-center py-2 rounded-lg text-xs font-bold transition-all"
                  style={ladderResetEnabled
                    ? { background: "hsl(258 82% 64% / 0.15)", color: "hsl(var(--primary))", border: "1px solid hsl(258 82% 64% / 0.4)" }
                    : { background: "hsl(var(--muted))", color: "hsl(var(--muted-foreground))", border: "1px solid hsl(var(--border))" }}
                >
                  {ladderResetEnabled ? "ON" : "OFF"}
                </button>
              </div>

              <div>
                <label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground mb-1 block">
                  {side === "BUY" ? "New Restart" : "Bottom Out"}
                </label>
                <div className="relative">
                  <select
                    className="w-full appearance-none rounded-lg px-2 py-2 pr-6 text-sm font-mono bg-input border border-border focus:outline-none focus:ring-1 focus:ring-ring transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                    value={stepSizeIncrement}
                    onChange={(e) => setStepSizeIncrement(e.target.value)}
                    disabled={!ladderResetEnabled || !(priceNum > 0 && stepSizeNum > 0)}
                  >
                    {priceNum > 0 && stepSizeNum > 0 ? (
                      topBottomOutLevels.map((lvl) => {
                        const levelPrice = side === "BUY"
                          ? priceNum + stepSizeNum * lvl
                          : priceNum - stepSizeNum * lvl;
                        return (
                      ...
                      <option value={stepSizeIncrement}>Enter price</option>
                    )}
                  </select>
            ...
            <p className="text-[10px] text-muted-foreground -mt-1">
              {!ladderResetEnabled
                ? "Ladder Reset is off — Top Out/Bottom Out is ignored, nothing resets."
                : stepSizeNum > 0 && stepSizeIncrement
                ? `If price runs ${(stepSizeNum * Math.max(1, Math.min(100, parseInt(stepSizeIncrement) || 0))).toFixed(2)} pts away with nothing live, the whole chain resets closer to market.`
  ...
  <p className="text-[10px] text-muted-foreground -mt-2">
    Limits every {stepSizeNum || "…"} pts {side === "BUY" ? "below" : "above"} entry · TP = limit {side === "BUY" ? "+" : "−"} {tpPointsNum || "…"} pts.
            ...
            <div className="space-y-2 pt-1">
              <button onClick={handleExecute} disabled={isExecuting || isPunching} className="w-full py-3 rounded-xl font-bold text-sm tracking-wide transition-all disabled:opacity-50 disabled:cursor-not-allowed"
...
//         } else {
//           const stepSizeIncrementNum = Math.max(1, Math.min(100, parseInt(stepSizeIncrement) || 2));
//           await runAutoPunch(symbol.toUpperCase(), side, ep, baseQty, { orderCount, stepSize: stepSizeNum, tpPoints: tpPointsNum, stepSizeIncrement: stepSizeIncrementNum }, { orderId: result.orderId }, doubleQtyEnabled);
//         }
//       }
//       void refetchOrders();
//       void refetchPositions();
...
//                     value={stepSizeIncrement}
//                     onChange={(e) => setStepSizeIncrement(e.target.value)}
//                     disabled={!(priceNum > 0 && stepSizeNum > 0)}
//                   >
//                     {priceNum > 0 && stepSizeNum > 0 ? (
//                       topBottomOutLevels.map((lvl) => {
//                         const levelPrice = side === "BUY"
